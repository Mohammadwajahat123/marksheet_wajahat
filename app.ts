#! /usr/bin/env node
let names: string = 'MOHAMMAD WAJAHAT';
let roll: string = '123';
let standard: string = '12';
let math: number = 60; // Placeholder value
let urdu: number = 70; // Placeholder value
let physic: number = 80; // Placeholder value
let chemistry: number = 50; // Placeholder value
let english: number = 60; // Placeholder value
...
function calculateResults(
    ...
    chemistry: number,
    english: number
): { obtainedMarks: number, percentage: number } {
...
function determineGradeAndRemarks(percentage: number): { grade: string, remarks: string } {
    let grade: string;
    let remarks: string;
